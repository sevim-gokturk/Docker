...
# Define Redis client (Redis service is running on host 'redis')
cache = redis.Redis(host='redis', port=6379)


def get_hit_count():
    """
    Retrieve and increment a counter stored in Redis.
    If a connection error occurs, retry a limited number of times.
    """
    retries = 5  # Maximum number of retry attempts
    while True:
        try:
            return cache.incr('hits')  # Increment the 'hits' key in Redis
        except redis.exceptions.ConnectionError as exc:
            if retries == 0:
                raise exc  # Raise an exception if connection fails after retries
            retries -= 1
...
def hello():
    """
    Respond to HTTP requests to the root endpoint.
    Displays the number of times the page has been visited.
    """
    count = get_hit_count()
    return f'Hello World! I have been seen {count} times.\n'

# Typically, app.run() is used to start the Flask application,
# but in production, it should be run with a WSGI server.
